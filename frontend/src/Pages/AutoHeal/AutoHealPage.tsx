...
const AutoHealPage = observer(() => {
  ...
  <section className={styles.grid}><article className={styles.card}><h2>Rules list</h2>{state === 'loading' ? <p>Loading…</p> : null}{state === 'error' ? <p>Error loading rules. {error}</p> : null}{state === 'empty' ? <p>Empty state: no rules yet.</p> : null}{rules.map((r) => <div key={r.id}>{r.name} · {r.metric} {r.condition} {r.threshold} · {r.action} · retries {r.retry_limit} · cooldown {r.cooldown_seconds}s</div>)}</article>
  <article className={styles.card}><h2>Execution history</h2>{history.length === 0 ? <p>Empty state.</p> : history.map((h) => <div key={h.id}>{new Date(h.created_at).toLocaleString()} · {h.rule_name} · {h.status} · {h.message}</div>)}</article></section></main></div>;
});
