...
const SecretsPage = observer(() => {
  ...
  const load = async () => {
    ...
    const [listResp, auditResp] = await Promise.all([
      fetch(`${API_BASE}/servers/${serverId}/secrets`, { headers: authHeaders }),
      fetch(`${API_BASE}/servers/${serverId}/secrets/audit`, { headers: authHeaders }),
    ]);
    if (listResp.ok) {
    ...
    <div className={styles.page}>
      ...
      <section className={styles.card}>
        ...
        <div className={styles.formRow}>
          ...
          <select className={styles.select} value={secretType} onChange={e => setSecretType(e.target.value as any)}>
            {SecretTypes.map(type => <option key={type} value={type}>{type}</option>)}
          </select>
          <input className={styles.input} placeholder='Service scope (optional)' value={serviceScope} onChange={e => setServiceScope(e.target.value)} />
          <input className={styles.input} type='datetime-local' value={expiresAt} onChange={e => setExpiresAt(e.target.value)} />
        </div>
        <textarea className={styles.editor} placeholder='Secret value (never fully shown later)' value={value} onChange={e => setValue(e.target.value)} />
        <div className={styles.row}>
          <button
            type='button'
            className={styles.btnPrimary}
            onClick={() => void createSecret().catch((e) => toastStore.push('error', e instanceof Error ? e.message : 'Create failed', 'Secrets Vault'))}
          >
            Create
          </button>
        </div>
            ...
            <tbody>
              {rows.map(row => (
                <tr key={row.id}>
                  ...
                  <td>{row.last_rotated_at ? new Date(row.last_rotated_at).toLocaleString() : 'never'}</td>
                  <td>{row.expires_at ? new Date(row.expires_at).toLocaleString() : '—'}</td>
                  <td>{row.usage_count}{row.last_used_at ? ` (${new Date(row.last_used_at).toLocaleString()})` : ''}</td>
                  <td className={styles.actions}>
                    <button type='button' className={styles.btn} onClick={() => void revealPartial(row.id).catch((e) => toastStore.push('error', e instanceof Error ? e.message : 'Reveal failed', 'Secrets Vault'))}>Partial reveal</button>
                    <button type='button' className={styles.btn} onClick={() => void rotateSecret(row.id).catch((e) => toastStore.push('error', e instanceof Error ? e.message : 'Rotate failed', 'Secrets Vault'))}>Rotate</button>
                    <button type='button' className={styles.btnDanger} onClick={() => setRevokeId(row.id)}>Revoke</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className={styles.logs}>
          {audit.map(item => (
            <div key={item.id} className={styles.logLine}>
              {new Date(item.created_at).toLocaleString()} — {item.action} — {item.target || 'n/a'} — {item.message}
            </div>
          ))}
        </div>
      </section>
      ...
      <Confirm
        ...
        onConfirm={() => {
          ...
          void revokeSecret(revokeId)
            ...
            .catch((e) => toastStore.push('error', e instanceof Error ? e.message : 'Revoke failed', 'Secrets Vault'))
