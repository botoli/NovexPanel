...
func main() {
	cfg := agentConfig{}
	flag.StringVar(&cfg.BackendURL, "backend", "ws://localhost:8380", "Backend WebSocket base URL")
	flag.StringVar(&cfg.Token, "token", "", "Agent token generated by backend")
	flag.StringVar(&cfg.Name, "name", "", "Optional server name shown in panel")
	flag.DurationVar(&cfg.MetricsInterval, "metrics-interval", 2*time.Second, "Metrics push interval")
	flag.BoolVar(&cfg.InsecureTLS, "insecure-tls", false, "Skip TLS cert verification (dev only)")
	flag.Parse()
